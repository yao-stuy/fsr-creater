...
| | `--fixed-pins` | always 16 pins: 1–8 = columns, 9–16 = rows, unused = NC — one cable/readout board fits any array up to 8×8 (ZIF pitch defaults to 1.25 mm) |
...
| | `--connector-rows {1,2}` | split the pins into two equal rows (e.g. 2×8 header for 16 pins); pin count must divide evenly (default 1) |
| | `--connector-numbering {zigzag,straight}` | dual-row numbering: zigzag = pins 1+2 share a column (IDC convention); straight = pins 1..n/2 across one row, rest across the other |
